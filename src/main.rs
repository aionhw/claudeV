...
impl App {
    fn new() -> Self {
        App {
            ...
            cursor: None,
            focus: Focus::ModTree,
            status: "Tab=switch focus  a=add signal  drag signal->wave  d=remove  +/-=zoom  q=quit"
                .into(),
            left_w: LEFT_W,
            name_w: NAME_W,
            mod_split: MOD_SPLIT,
    ...
    fn max_time(&self) -> f64 {
        self.vcd
            .as_ref()
            .map(|v| v.max_time as f64)
            .unwrap_or(100.0)
    }
    fn view_end(&self) -> f64 {
        self.view_start + self.max_time() / self.zoom
    }
    ...
    fn wave_h(&self) -> i16 {
        ((WAVE_H as f64) * self.size_scale()).round() as i16
    }

    fn row_h(&self) -> i16 {
        ((ROW_H as f64) * self.size_scale()).round() as i16
    }

    fn sig_h(&self) -> i16 {
        ((SIG_H as f64) * self.size_scale()).round() as i16
    }
    ...
    fn wave_vis_rows(&self, win_h: i16) -> usize {
        ((win_h - TOP_H - STATUS_H - MARKER_H - RULER_H) / self.wave_h()).max(1) as usize
    }
                ...
                if seen.insert(path.clone()) {
                    ...
                    if let Some(p) = parent {
                        if let Some(pn) = self.mod_nodes.get_mut(&p) {
                            if !pn.children.contains(&path) {
                                pn.children.push(path);
                            }
                        }
                    } else {
                        if !self.mod_roots.contains(&path) {
                            self.mod_roots.push(path);
                        }
                    }
                }
            }
    ...
    fn signal_label(&self, sig: &vcd_parser::Signal) -> String {
        let base = if self.show_full_names {
            &sig.full_name
        } else {
            &sig.name
        };
        if sig.width > 1 {
            format!("{}[{}:0]", base, sig.width - 1)
        } else {
            base.to_string()
        }
    }
    ...
    fn sig_content_cols(&self) -> usize {
        ...
        self.sig_rows
            ...
            .map(|&si| {
                ...
                self.signal_label(sig).chars().count()
            })
            .max()
            .unwrap_or(0)
    }
    ...
    // ── Pinning ───────────────────────────────────────────────────────────────
    fn is_pinned(&self, si: usize) -> bool {
        ...
        let to_add: Vec<usize> = vcd
            ...
            .filter(|(_, sig)| {
                show_all
                    ...
                    || sig.scope == scope
                    || (!scope.is_empty()
                        && sig.scope.starts_with(scope)
                        && sig.scope.as_bytes().get(scope.len()) == Some(&b'.'))
            })
            .map(|(i, _)| i)
            .collect();
        for si in to_add {
    ...
    fn toggle_current_signal_selection(&mut self) {
        if let Some(&si) = self.sig_rows.get(self.sig_sel) {
            if !self.sig_selected.insert(si) {
                self.sig_selected.remove(&si);
            }
        }
    }

    fn toggle_current_wave_selection(&mut self) {
        if let Some(sig_idx) = self.wave_row_signal(self.wave_sel) {
            if !self.wave_selected.insert(sig_idx) {
                self.wave_selected.remove(&sig_idx);
            }
        }
    }

    fn wave_row_signal(&self, row_idx: usize) -> Option<usize> {
        match self.wave_rows.get(row_idx) {
            Some(WaveRow::Signal { sig_idx }) | Some(WaveRow::BitSlice { sig_idx, .. }) => {
                Some(*sig_idx)
            }
            None => None,
        }
    ...
    fn select_sig_range_to(&mut self, row_idx: usize) {
        let a = self.sig_sel.min(row_idx);
        let b = self.sig_sel.max(row_idx);
        for ri in a..=b {
            if let Some(&si) = self.sig_rows.get(ri) {
                self.sig_selected.insert(si);
            }
        }
    }

    fn select_wave_range_to(&mut self, row_idx: usize) {
        let a = self.wave_sel.min(row_idx);
        let b = self.wave_sel.max(row_idx);
        for ri in a..=b {
            if let Some(si) = self.wave_row_signal(ri) {
                self.wave_selected.insert(si);
            }
        }
    }
    ...
    fn scroll_mod(&mut self, win_h: i16) {
        ...
        } else if self.mod_sel >= self.mod_scroll + vr {
            self.mod_scroll = self.mod_sel + 1 - vr;
        }
    }
    ...
    fn scroll_sig(&mut self, win_h: i16) {
        ...
        if self.sig_sel < self.sig_scroll {
            self.sig_scroll = self.sig_sel;
        } else if self.sig_sel >= self.sig_scroll + vr {
            self.sig_scroll = self.sig_sel + 1 - vr;
        }
        self.clamp_sig_hscroll();
    }
    ...
    fn scroll_wave(&mut self, win_h: i16) {
        ...
        if self.wave_sel < self.wave_scroll {
            self.wave_scroll = self.wave_sel;
        } else if self.wave_sel >= self.wave_scroll + vr {
            self.wave_scroll = self.wave_sel + 1 - vr;
        }
    }

    // ── Edge jump ─────────────────────────────────────────────────────────────
    fn jump_edge(&mut self, fwd: bool) {
        let (Some(vcd), Some(cursor)) = (&self.vcd, self.cursor) else {
            return;
        };
        let si = match self.wave_rows.get(self.wave_sel) {
            Some(WaveRow::Signal { sig_idx }) | Some(WaveRow::BitSlice { sig_idx, .. }) => *sig_idx,
            None => return,
        };
        let Some(changes) = vcd.changes.get(&vcd.signals[si].id) else {
            return;
        };
        let t = cursor as u64;
        let nt = if fwd {
        ...
        if let Some(nt) = nt {
            self.cursor = Some(nt);
            if nt > self.view_end() {
                self.view_start = (nt - self.max_time() / self.zoom * 0.1).max(0.0);
                self.clamp_view();
            }
            if nt < self.view_start {
                self.view_start = (nt - self.max_time() / self.zoom * 0.1).max(0.0);
            }
        }
    }
    ...
    fn hit_menu(&self, mouse_x: i16, mouse_y: i16) -> Option<usize> {
        ...
        for (idx, label) in MENU_ITEMS.iter().enumerate() {
            let x = self.menu_x(idx);
            let item_w = menu_item_w(label);
    ...
    fn menu_x(&self, menu_idx: usize) -> i16 {
        let mut x = 6;
        for label in MENU_ITEMS.iter().take(menu_idx) {
            x += menu_item_w(label);
        }
        x
    }

    fn dropdown_geometry(&self, menu_idx: usize, items: &[&str]) -> Option<(i16, i16, i16, i16)> {
        if self.selected_menu != Some(menu_idx) {
            return None;
        }
        let x = (self.menu_x(menu_idx) - 4).max(0);
        let y = HEADER_H;
        let w = items.iter().map(|item| menu_tw(item)).max().unwrap_or(0) + 20;
        let h = items.len() as i16 * self.row_h();
        Some((x, y, w, h))
    }

    fn file_menu_geometry(&self) -> Option<(i16, i16, i16, i16)> {
        self.dropdown_geometry(0, &FILE_MENU_ITEMS)
    }

    fn view_menu_geometry(&self) -> Option<(i16, i16, i16, i16)> {
        self.dropdown_geometry(3, &VIEW_MENU_ITEMS)
    }
    ...
    fn hit_file_menu(&self, mouse_x: i16, mouse_y: i16) -> Option<usize> {
        ...
        Some(((mouse_y - y) / self.row_h()) as usize)
    }

    fn hit_view_menu(&self, mouse_x: i16, mouse_y: i16) -> Option<usize> {
        let (x, y, w, h) = self.view_menu_geometry()?;
        if mouse_x < x || mouse_x >= x + w || mouse_y < y || mouse_y >= y + h {
            return None;
        }
        Some(((mouse_y - y) / self.row_h()) as usize)
    ...
    fn activate_view_menu(&mut self, item_idx: usize) -> MenuAction {
        self.selected_menu = None;
        match item_idx {
            0 => {
    ...
    fn hit_sig_row(&self, mouse_x: i16, mouse_y: i16, win_h: i16) -> Option<usize> {
        ...
        let sig_y = TOP_H + mod_h;
        let row_h = self.row_h();
        let sig_row_h = self.sig_h();
        let rows_y = sig_y + row_h;
        let rows_h = (sig_h - row_h - SB).max(1);
        if mouse_y < rows_y || mouse_y >= rows_y + rows_h {
            return None;
        }
        let vis_row = ((mouse_y - rows_y) / sig_row_h) as usize;
        ...
        if let Some(vcd) = &self.vcd {
            let label = match self.wave_rows.get(row_idx) {
                ...
                Some(WaveRow::BitSlice { sig_idx, bit }) => {
                    format!("{}[{}]", vcd.signals[*sig_idx].full_name, bit)
                }
                None => String::new(),
            };
            if !label.is_empty() {
    ...
    fn handle_keysym(&mut self, ks: u32, win_h: i16) -> bool {
            ...
            match ks {
                ...
                _ => {
                    if (0x20..=0x7E).contains(&ks) {
                        self.filter_text
                            .push(char::from_u32(ks).unwrap_or_default());
                        self.rebuild_sig_rows();
                        self.status = format!("Filter: {}", self.filter_text);
                    }
                    return false;
        ...
        match ks {
            ...
            0x2B | 0x3D => {
                self.zoom_by(2.0, None);
                return false;
            }
            0x2D | 0x5F => {
                self.zoom_by(0.5, None);
                return false;
            }
            0x30 | 0x66 | 0x46 => {
                self.fit_all();
            ...
            k if k == XK_LEFT || k == 0x68 => {
                self.view_start = (self.view_start - self.max_time() / self.zoom * 0.1).max(0.0);
                return false;
            }
            k if k == XK_RIGHT || k == 0x6C => {
                self.view_start += self.max_time() / self.zoom * 0.1;
                self.clamp_view();
                return false;
            }
            0x48 => {
            ...
            0x4C => {
                self.view_start = (self.max_time() - self.max_time() / self.zoom).max(0.0);
                return false;
            }
            0x63 => {
                self.cursor = Some(self.view_start + (self.max_time() / self.zoom) / 2.0);
                return false;
            }
            0x43 => {
                self.cursor = None;
            ...
            0x6D => {
                let t = self
                    .cursor
                    .unwrap_or(self.view_start + (self.max_time() / self.zoom) / 2.0);
                self.markers[self.active_marker] = Some(t);
                self.status = format!(
                    "Marker {} set to {:.0}",
                    if self.active_marker == 0 { "A" } else { "B" },
                    t
                );
                return false;
            }
            ...
            0x5B => {
                if let Some(t) = self.cursor {
                    self.cursor = Some((t - self.max_time() / self.zoom * 0.02).max(0.0));
                }
                return false;
            }
            0x5D => {
                if let Some(t) = self.cursor {
                    self.cursor =
                        Some((t + self.max_time() / self.zoom * 0.02).min(self.max_time()));
                }
                return false;
            }
            0x6E => {
            ...
            0x3F => {
                self.status = "Tab=focus | space=multi-select | t=theme z=size p=path/name | m=set marker M=next D=clear | SIG a/A | WAVE d J/K e | VIEW +/- f h/l c n/N q".into();
                return false;
            }
            0x70 | 0x50 => {
            ...
            0x7A | 0x5A => {
                self.size_mode = (self.size_mode + 1) % 4;
                self.status = format!("Size: {} ({:.2}x)", self.size_label(), self.size_scale());
                self.scroll_wave(win_h);
                return false;
            }
            0x74 | 0x54 => {
    ...
    fn key_mod(&mut self, ks: u32, win_h: i16) {
        ...
        match ks {
            ...
            k if k == XK_PAGE_UP => {
                let mod_h = ((win_h - TOP_H - STATUS_H - MARKER_H) as f32 * self.mod_split) as i16;
                let vr = self.mod_vis_rows(mod_h);
                self.mod_sel = self.mod_sel.saturating_sub(vr);
                self.scroll_mod(win_h);
                self.rebuild_sig_rows();
            }
            k if k == XK_PAGE_DOWN => {
                let mod_h = ((win_h - TOP_H - STATUS_H - MARKER_H) as f32 * self.mod_split) as i16;
                let vr = self.mod_vis_rows(mod_h);
                self.mod_sel = (self.mod_sel + vr).min(n.saturating_sub(1));
                self.scroll_mod(win_h);
                self.rebuild_sig_rows();
            }
    ...
    fn key_sig(&mut self, ks: u32, win_h: i16) {
        ...
        match ks {
            ...
            k if k == XK_PAGE_UP => {
                ...
                self.sig_sel = self.sig_sel.saturating_sub(vr);
                self.scroll_sig(win_h);
            }
            k if k == XK_PAGE_DOWN => {
                let body_h = win_h - TOP_H - STATUS_H - MARKER_H;
                let mod_h = (body_h as f32 * self.mod_split) as i16;
                let vr = self.sig_vis_rows(body_h - mod_h);
                self.sig_sel = (self.sig_sel + vr).min(n.saturating_sub(1));
                self.scroll_sig(win_h);
            }
            // a / Enter — toggle pin
            k if k == XK_RETURN || k == 0x61 => {
                let targets: Vec<usize> = if self.sig_selected.is_empty() {
                    self.sig_rows
                        .get(self.sig_sel)
                        .copied()
                        .into_iter()
                    ...
                    self.sig_selected.iter().copied().collect()
                };
                for si in &targets {
                    self.toggle_pin(*si);
                }
                if targets.len() == 1 {
                    ...
                    let fname = self
                        .vcd
                        .as_ref()
                        .map(|v| v.signals[si].full_name.clone())
                        .unwrap_or_default();
                    self.status = format!(
                        "{}: {}",
                        if self.is_pinned(si) {
                    ...
                    self.status = format!("Toggled {} selected signals", targets.len());
                }
            }
            k if k == XK_SPACE => {
                self.toggle_current_signal_selection();
                self.status = format!("Selected signals: {}", self.sig_selected.len());
            }
    ...
    fn key_wave(&mut self, ks: u32, win_h: i16) {
        ...
        match ks {
            ...
            k if k == XK_PAGE_UP => {
                let vr = self.wave_vis_rows(win_h);
                self.wave_sel = self.wave_sel.saturating_sub(vr);
                self.scroll_wave(win_h);
            }
            k if k == XK_PAGE_DOWN => {
                let vr = self.wave_vis_rows(win_h);
                self.wave_sel = (self.wave_sel + vr).min(n.saturating_sub(1));
                self.scroll_wave(win_h);
            }
            // d / Del / Backspace — remove
            k if k == 0x64 || k == XK_DELETE || k == XK_BACKSPACE => {
                if self.wave_selected.is_empty() {
                    match self.wave_rows.get(self.wave_sel).cloned() {
                        Some(WaveRow::Signal { sig_idx }) => {
                            self.unpin(sig_idx);
                            self.wave_sel =
                                self.wave_sel.min(self.wave_rows.len().saturating_sub(1));
                        }
                        Some(WaveRow::BitSlice { sig_idx, .. }) => {
                            self.wave_expanded.remove(&sig_idx);
                            self.rebuild_wave();
                    ...
                    let removed: Vec<usize> = self.wave_selected.iter().copied().collect();
                    for si in removed {
                        self.unpin(si);
                    }
                    self.wave_selected.clear();
            ...
            k if k == XK_SPACE => {
                self.toggle_current_wave_selection();
                self.status = format!("Selected waveforms: {}", self.wave_selected.len());
            }
            // J — move down
            0x4A => {
                if let Some(WaveRow::Signal { sig_idx }) =
                    self.wave_rows.get(self.wave_sel).cloned()
                {
                    if let Some(pos) = self.pinned.iter().position(|&s| s == sig_idx) {
                        if pos + 1 < self.pinned.len() {
                            self.pinned.swap(pos, pos + 1);
                            self.rebuild_wave();
                            if let Some(p) = self.wave_rows.iter().position(
                                |r| matches!(r, WaveRow::Signal{sig_idx:s} if *s==sig_idx),
                            ) {
                                self.wave_sel = p;
                            }
                        }
            ...
            _ => {
                ...
                if let Some(vcd) = &self.vcd {
                    let s = match self.wave_rows.get(self.wave_sel) {
                        Some(WaveRow::Signal { sig_idx }) => {
                            vcd.signals[*sig_idx].full_name.clone()
                        }
                        Some(WaveRow::BitSlice { sig_idx, bit }) => {
                            format!("{}[{}]", vcd.signals[*sig_idx].full_name, bit)
                        }
    ...
    fn handle_button(
        ...
        if button == 1 {
            ...
            if let Some(item_idx) = self.hit_view_menu(mouse_x, mouse_y) {
                return self.activate_view_menu(item_idx);
            }
            ...
            if mouse_y >= TOP_H && mouse_y < marker_y {
                if mouse_x < self.left_w {
                    ...
                    if let Some(row_idx) = self.hit_sig_row(mouse_x, mouse_y, win_h) {
                        ...
                        if shift {
                            self.select_sig_range_to(row_idx);
                        }
                        self.select_sig_row(row_idx, win_h);
                        if let Some(&sig_idx) = self.sig_rows.get(row_idx) {
                            if ctrl {
                                if !self.sig_selected.insert(sig_idx) {
                                    self.sig_selected.remove(&sig_idx);
                                }
                                self.status =
                                    format!("Selected signals: {}", self.sig_selected.len());
                            }
                ...
                let vis_row = ((mouse_y - (TOP_H + RULER_H)) / self.wave_h()) as usize;
                let row_idx = self.wave_scroll + vis_row;
                if row_idx < self.wave_rows.len() {
                    let ctrl = state.contains(KeyButMask::CONTROL);
                    let shift = state.contains(KeyButMask::SHIFT);
                    if shift {
                        self.select_wave_range_to(row_idx);
                    }
                    self.select_wave_row(row_idx, win_h);
                    if ctrl {
                        self.toggle_current_wave_selection();
                    ...
                    if mouse_x >= self.left_w + self.name_w - FW * 3
        ...
        let in_sig_panel =
            mouse_x >= 0 && mouse_x < self.left_w && mouse_y >= sig_y && mouse_y < marker_y;
        if in_sig_panel {
            let shift = state.contains(KeyButMask::SHIFT);
            let body_h = win_h - TOP_H - STATUS_H - MARKER_H;
            let mod_h = (body_h as f32 * self.mod_split) as i16;
    ...
    fn handle_button_release(&mut self, mouse_x: i16, mouse_y: i16, win_h: i16) {
        if self.drag_mode == DragMode::SigToWave {
            if let Some(sig_idx) = self.drag_sig {
                let marker_y = win_h - STATUS_H - MARKER_H;
                let drop_in_wave =
                    mouse_x >= self.left_w && mouse_y >= TOP_H + RULER_H && mouse_y < marker_y;
                if drop_in_wave {
                    self.pin(sig_idx);
...
fn set_active_theme(theme: usize) {
    ACTIVE_THEME.store(theme, Ordering::Relaxed);
}

fn set_active_font_ascent(ascent: u16) {
    ACTIVE_FONT_ASCENT.store(ascent.max(1) as usize, Ordering::Relaxed);
}

fn theme_color(color: u32) -> u32 {
    match ACTIVE_THEME.load(Ordering::Relaxed) {
        1 => match color {
            ...
            C_PATH => 0x9AA6B6,
            C_VALUE_BG => 0x12161C,
            C_OVERVIEW_WIN => 0x3A638C,
            C_OVERVIEW_ALL => 0x232A34,
            C_SPLIT => 0x47515F,
            other => other,
        },
        2 => match color {
            ...
            C_PATH => 0xA89B75,
            C_VALUE_BG => 0x0F0E09,
            C_OVERVIEW_WIN => 0x6F5C28,
            C_OVERVIEW_ALL => 0x272216,
            C_SPLIT => 0x51472A,
            other => other,
        },
        3 => match color {
            ...
            C_PATH => 0x7893A0,
            C_VALUE_BG => 0x071016,
            C_OVERVIEW_WIN => 0x1F6E8F,
            C_OVERVIEW_ALL => 0x132631,
            C_SPLIT => 0x2D5362,
            other => other,
        },
        _ => color,
...
fn txt(
    ...
    let ascent = ACTIVE_FONT_ASCENT.load(Ordering::Relaxed) as i16;
    let _ = conn.change_gc(
        gc,
        &ChangeGCAux::new()
    ...
    let _ = conn.image_text8(
        ...
        x,
        y + ascent,
        &s.bytes().take(255).collect::<Vec<u8>>(),
    );
}
...
fn menu_tw(s: &str) -> i16 {
    s.len() as i16 * 9
}

fn menu_item_w(label: &str) -> i16 {
    menu_tw(label) + 20
}
...
fn render_marker_dock(
    ...
    txt(
        ...
        gc,
        font,
        if app.active_marker == 0 { C_CUR } else { C_LBL },
        C_TOOLBAR,
        x + 6,
        y + 20,
        &format!("A  {}", fmt_marker(vcd, a)),
    );
    txt(
        conn,
        pix,
        gc,
        font,
        if app.active_marker == 1 { C_CUR } else { C_LBL },
        C_TOOLBAR,
        x + 120,
        y + 20,
        &format!("B  {}", fmt_marker(vcd, b)),
    );
    txt(
        conn,
        pix,
...
fn render(
    ...
    let font_ascent = match app.size_mode {
        1 => 13,
        2 => 16,
        3 => 20,
        _ => FA as u16,
    };
    set_active_font_ascent(font_ascent);
    ...
    for (idx, label) in MENU_ITEMS.iter().enumerate() {
        ...
        txt(conn, pix, gc, menu_font, fg, bg, menu_x, 4, label);
        menu_x += item_w;
    }
    set_active_font_ascent(font_ascent);
    // Toolbar
    ...
    let toolbar = if let Some(vcd) = &app.vcd {
        let cur = app
            .cursor
            .map(|t| format!("cursor {:.0}{}", t, vcd.timescale))
            .unwrap_or_else(|| "cursor off".into());
        let delta = match (app.markers[0], app.markers[1]) {
            (Some(a), Some(b)) => format!("delta {:.0}{}", (b - a).abs(), vcd.timescale),
            _ => "delta --".into(),
        };
        format!(
    ...
    let bdr = if app.focus != Focus::Wave {
        C_BDR_FOCUS
    } else {
        C_BDR
    };
    ...
    if app.file_browser_active {
        ...
        let filter_lbl = if app.filter_text.is_empty() {
            "all".to_string()
        } else {
        ...
        let sig_avail = (sig_rows_h / sig_row_h).max(0) as usize;
        ...
        for (ri, &si) in app
            ...
            .take(sig_avail)
        {
            let ry = sig_rows_y + (ri - sscroll) as i16 * sig_row_h;
            if ry + sig_row_h > sig_rows_y + sig_rows_h {
                break;
            }
            let is_sel = ri == app.sig_sel && app.focus == Focus::SigList;
            let multi_sel = app.sig_selected.contains(&si);
            let pinned = app.is_pinned(si);
            let bg = if is_sel || multi_sel {
                C_SEL_SIG
            } else {
                C_PANEL
            };
            fill(
                ...
                0,
                ry,
                sig_content_w as u16,
                sig_row_h as u16,
            );

            if let Some(vcd) = &app.vcd {
                let sig = &vcd.signals[si];
                let name = app.signal_label(sig);
                ...
                txt(
                    ...
                    bg,
                    16,
                    ry + (sig_row_h - ACTIVE_FONT_ASCENT.load(Ordering::Relaxed) as i16) / 2,
                    &slice_text(&name, shscroll, sig_text_cols),
                );
                ...
                if is_sel || multi_sel {
                    fill(conn, pix, gc, C_PINNED, 0, ry, 2, sig_row_h as u16);
                }
            }
            seg(
                conn,
        ...
        let vthumb_h = if max_vscroll == 0 {
            ...
            ((sig_rows_h as f64 * vis_rows as f64 / total_rows as f64).round() as i16)
                .clamp(10, sig_rows_h)
        };
        let vthumb_y = if max_vscroll == 0 {
            ...
            sig_rows_y
                + (((sig_rows_h - vthumb_h) as f64 * sscroll as f64 / max_vscroll as f64).round()
                    as i16)
        };
        fill(
            conn,
        ...
        let hthumb_w = if max_hscroll == 0 {
            ...
            ((sig_content_w as f64 * vis_cols as f64 / total_cols as f64).round() as i16)
                .clamp(16, sig_content_w)
        };
        let hthumb_x = if max_hscroll == 0 {
            0
        } else {
            (((sig_content_w - hthumb_w) as f64 * shscroll as f64 / max_hscroll as f64).round()
                as i16)
                .max(0)
        };
        fill(
    ...
    let Some(vcd) = &app.vcd else {
        txt(
            ...
            wx + 50,
            TOP_H + body_h / 2,
            "Select a scope, add signals, then inspect waveforms",
        );
        render_file_menu(conn, pix, gc, menu_font, app);
        return;
    };

    // Name column divider
    let wbdr = if app.focus == Focus::Wave {
        C_BDR_FOCUS
    } else {
        C_BDR
    };
    ...
    // End-of-sim marker
    let t0e = app.view_start;
    let t1e = app.view_end();
        ...
        .take(max_rows)
    {
        ...
        if ry + wave_h > base_y + avail_h {
            break;
        }
        let is_sel = ri == app.wave_sel;
        let row_sig = app.wave_row_signal(ri);
        let multi_sel = row_sig
            .map(|si| app.wave_selected.contains(&si))
            .unwrap_or(false);
        let wbg = if is_sel || multi_sel {
            C_SEL_WAVE
        } else if ri % 2 == 0 {
            C_BG
        } else {
        ...
        match row {
            WaveRow::Signal { sig_idx } => {
                ...
                // Name column
                let nbg = if is_sel && app.focus == Focus::Wave {
                    C_SEL_WAVE
                } else {
                    C_PANEL
                };
                ...
                let short_y = (ry + wave_h - font_ascent as i16 - 2).max(ry + 2);
                txt(
                    conn,
                    pix,
                    gc,
                ...
                txt(
                    ...
                    gc,
                    font,
                    if app.cursor.is_some() { C_CUR } else { C_BUS },
                    C_VALUE_BG,
                    value_x + 4,
                    short_y,
                    &trunc_l(&dv, vmax),
...
fn render_file_menu(conn: &RustConnection, pix: u32, gc: u32, font: u32, app: &App) {
    render_dropdown_menu(
        ...
        font,
        app,
        app.file_menu_geometry(),
        &FILE_MENU_ITEMS,
        |item| *item == "Reload" && app.file_path.is_none(),
    );
    render_dropdown_menu(
        conn,
        pix,
...
fn render_dropdown_menu(
    ...
    font: u32,
    app: &App,
    geometry: Option<(i16, i16, i16, i16)>,
    items: &[&str],
    disabled_item: impl Fn(&&str) -> bool,
) {
    if let Some((mx, my, mw, mh)) = geometry {
        let prev_ascent = ACTIVE_FONT_ASCENT.load(Ordering::Relaxed);
        set_active_font_ascent(13);
        ...
        for (idx, item) in items.iter().enumerate() {
            ...
            let disabled = disabled_item(item);
        ...
        ACTIVE_FONT_ASCENT.store(prev_ascent, Ordering::Relaxed);
    }
}
...
fn render_ruler(
    ...
    let to_x =
        |t: f64| -> i16 { x + ((t - t0) / range * w as f64).clamp(0.0, w as f64 - 1.0) as i16 };
    txt(
        conn,
        pix,
        gc,
    ...
    if let Some(ct) = cursor {
        if ct >= t0 && ct <= t1 {
            fill(conn, pix, gc, C_CUR, to_x(ct) - 1, y, 3, RULER_H as u16);
        }
    }
    seg(
        conn,
...
fn render_wave(
    ...
    let to_x =
        |t: f64| -> i16 { x + ((t - t0) / range * w as f64).clamp(0.0, w as f64 - 1.0) as i16 };
    let pad: i16 = ((wave_h as f64) * 0.18).round().clamp(5.0, 14.0) as i16;
    let mid = y + wave_h / 2;
    let hi = y + pad;
    ...
    if changes.is_empty() {
        ...
        for c in changes {
            if (c.time as f64) <= t0 {
                cur = c.value.chars().next().unwrap_or('x');
            } else {
                break;
            }
        }
        ...
        let fh = |conn: &RustConnection, col: u32, ss: &mut Vec<Segment>| {
            if !ss.is_empty() {
                segs(conn, pix, gc, col, ss);
                ss.clear();
            }
        };
        for px in 0..(w as usize) {
            ...
            let mut n = 0usize;
            let mut nc = cur;
            while ci < changes.len() && (changes[ci].time as f64) <= tx1 {
                nc = changes[ci].value.chars().next().unwrap_or('x');
                n += 1;
                ci += 1;
            }
            if n == 0 {
                if xz(cur) {
                    xs.push((vc(cur), sx, vy(cur), sx));
                } else {
                    let c = vc(cur);
                    if c != hc {
                        fh(conn, hc, &mut hs);
        ...
        fh(conn, hc, &mut hs);
        for (col, x1, yy, x2) in xs {
            dashed(conn, pix, gc, col, x1, yy, x2);
        }
    } else {
        let ii = changes.partition_point(|c| (c.time as f64) <= t0);
        let mut pv = if ii > 0 {
            changes[ii - 1].value.clone()
        } else {
            "x".repeat(width)
        };
        let mut pt = t0;
        for c in &changes[ii..] {
            let vt = c.time as f64;
            if vt > t1 {
                break;
            }
            draw_bus(conn, pix, gc, font, x, w, pt, vt, &pv, t0, t1, hi, mid, lo);
            pv = c.value.clone();
            pt = vt;
        }
        draw_bus(conn, pix, gc, font, x, w, pt, t1, &pv, t0, t1, hi, mid, lo);
    }
    if let Some(t) = cursor {
        if t >= t0 && t <= t1 {
            let cx = to_x(t);
            seg(conn, pix, gc, C_CUR, cx, y, cx, y + wave_h - 1);
        }
    }
}
...
fn draw_bus(
    ...
    if sw > 16 {
        let lbl = if is_x {
            ...
            u64::from_str_radix(val, 2)
                .map(|n| format!("{:#X}", n))
                .unwrap_or_else(|_| val[..val.len().min(6)].to_string())
        };
        let lw = tw(&lbl);
        let lx = x0 + n + (sw - n * 2 - lw) / 2;
        if lx > x0 && lx + lw < x1 {
            txt(conn, pix, gc, font, col, C_BG, lx, mid - 6, &lbl);
        }
    }
}
...
fn extract_bit(val: &str, bit: usize) -> String {
    ...
    let ch = if bit < len {
        ...
        match chars.first().copied().unwrap_or('x') {
            '0' | '1' => '0',
            'z' | 'Z' => 'z',
            _ => 'x',
        }
...
fn synth_bit_changes(changes: &[ValueChange], bit: usize) -> Vec<ValueChange> {
    changes
        ...
        .map(|vc| ValueChange {
            time: vc.time,
            value: extract_bit(&vc.value, bit),
        })
        .collect()
}
...
fn open_font_any(conn: &RustConnection, names: &[&[u8]]) -> Res<u32> {
    for name in names {
        ...
        if conn.open_font(font, *name)?.check().is_ok() {
            return Ok(font);
        }
    }
    Err("no usable X11 font found".into())
}
...
fn main() -> Res<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-h" | "--help" => {
                ...
                eprintln!(
                    "space/Ctrl-click/Shift-click=multi-select  p=full path/name  z=size  t=theme"
                );
                ...
                eprintln!(
                    "SIGNALS: j/k=nav  a/Enter=add/remove  A=add all in selected scope  drag->wave"
                );
                eprintln!(
                    "          (select '(all scopes)' in module tree for global signal selection)"
                );
    ...
    conn.create_window(
        ...
        screen.root_visual,
        &CreateWindowAux::new().background_pixel(C_BG).event_mask(
            EventMask::EXPOSURE
                | EventMask::KEY_PRESS
    ...
    let fonts = [
        open_font_any(&conn, &[b"fixed"])?,
        open_font_any(&conn, &[b"9x15", b"8x13", b"fixed"])?,
        open_font_any(&conn, &[b"10x20", b"9x15", b"fixed"])?,
        open_font_any(&conn, &[b"12x24", b"10x20", b"fixed"])?,
    ];
    let menu_font = open_font_any(&conn, &[b"9x15bold", b"8x13bold", b"7x13bold", b"fixed"])?;
    ...
    loop {
        ...
        match ev {
            ...
            Event::KeyPress(e) => {
                let ks = get_keysym(
                    &kbd.keysyms,
                    kbd.keysyms_per_keycode,
                    e.detail,
                    mkc,
                    e.state,
                );
                if app.handle_keysym(ks, wh as i16) {
                    break;
                }
                dirty = true;
            }
            ...
            Event::ClientMessage(e) => {
                if e.data.as_data32().first().copied() == Some(wmd) {
                    break;
                }
            }
        ...
        if dirty {
            let font = fonts[app.size_mode.min(fonts.len() - 1)];
            render(&conn, pix, gc, font, menu_font, ww, wh, &app);
...
mod tests {
    ...
    fn all_scope_lists_every_signal() {
        ...
        let all_idx = app
            .mod_rows
            .iter()
            .position(|path| path == ALL_SCOPE_PATH)
